...
-- Lemma 3.1.12
theorem MySet.pair_eq_union_singleton
    (a b : α) :
    (⦃a, b⦄ : MySet α) = ⦃a⦄ ∪ ⦃b⦄ := by
  sorry

theorem MySet.union_comm
    (A B : MySet α) :
    A ∪ B = B ∪ A := by
  sorry
...
theorem MySet.union_self
    (A : MySet α) :
    A ∪ A = A := by
  sorry

theorem MySet.union_empty
    (A : MySet α) :
    A ∪ ∅ = A := by
  sorry

theorem MySet.empty_union
    (A : MySet α) :
    ∅ ∪ A = A := by
  sorry
...
-- (f)
theorem MySet.inter_union_distrib
    (A B C : MySet α) :
    A ∩ (B ∪ C) = (A ∩ B) ∪ (A ∩ C) := by
  sorry

theorem MySet.union_inter_distrib
    (A B C : MySet α) :
    A ∪ (B ∩ C) = (A ∪ B) ∩ (A ∪ C) := by
  sorry
...
theorem MySet.inter_diff
    (X A : MySet α) :
    A ∩ (X \ A) = ∅ := by
  sorry

theorem MySet.diff_union
    (X A B : MySet α) :
    X \ (A ∪ B) = (X \ A) ∩ (X \ B) := by
  sorry

theorem MySet.diff_inter
    (X A B : MySet α) :
    X \ (A ∩ B) = (X \ A) ∪ (X \ B) := by
  sorry
