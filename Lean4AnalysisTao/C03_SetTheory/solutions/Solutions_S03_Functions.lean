...
example
  ...
  · constructor
    ...
    · intro x hxf hxf'
      ...
      have hauxf : ∀ (x : α) (hx : x ∈ f.domain), f.eval x hx ∈ g.domain := by
        intro x hx
        rw [← hfg]
...
theorem finv_f
  ...
  have hffi : f.codomain = (f.inv hf).domain :=
    aux₁ f hf
  ...
  have hfinvaux : ∀ (y : β) (hy : y ∈ f.codomain), finv y hy ∈ f.domain := by
    intro y hy
    dsimp only [finv]
...
theorem f_finv
  ...
  have hffi : (f.inv hf).codomain = f.domain :=
    aux₂ f hf
  ...
  have hfeq : y = f.eval ((f.inv hf).eval y hyfidom) hfiyfdom :=
...
theorem finv_bij
  ...
  · dsimp only [MyFun.isInjective]
    ...
    have hxfifdom : x ∈ (f.comp (f.inv hf) (aux₁ f hf)).domain := by
      dsimp only [MyFun.comp]
      dsimp only [MyFun.from_fun]
      exact hxfdom
  ...
  · constructor
    ...
    · intro x hxfiidom hxfdom
      ...
      have hfifiixfifx :
        ...
        have hxfiidom2 : x ∈ ((f.inv hf).inv hfi).domain := by
          dsimp only [MyFun.inv]
          dsimp only [MyFun.from_fun]
          exact hxfiidom
      ...
      rw [MyFun.comp.eval f (f.inv hf) (aux₁ f hf) x hxfifdom hfxfidom hxfifdom]
        at hfifiixfifx
      have hficopy : (f.inv hf).isBijective :=
        hfi
  ...
  · constructor
    ...
    · intro z hzgfidom hzfigidom
      ...
      have hgizfidom : (g.inv hg).eval z hzgidom ∈ (f.inv hf).domain := by
        have hdomeq : (f.inv hf).domain = (g.inv hg).codomain := by
          dsimp only [MyFun.inv]
          dsimp only [MyFun.from_fun]
          exact hfg
        rw [hdomeq]
      ...
      have hgizy : (g.inv hg).eval z hzgidom = y := by
        have hgygidom : g.eval y hygdom ∈ (g.inv hg).domain := by
          have hdomeq : (g.inv hg).domain = g.codomain := by
            dsimp only [MyFun.inv]
            dsimp only [MyFun.from_fun]
          rw [hdomeq]
      ...
      have hfiyx : (f.inv hf).eval y hyfidom = x := by
        have hfxgidom : f.eval x hxfdom ∈ (f.inv hf).domain := by
          have hdomeq : (f.inv hf).domain = f.codomain := by
            dsimp only [MyFun.inv]
            dsimp only [MyFun.from_fun]
          rw [hdomeq]
          exact MyFun.eval_codomain f x hxfdom
      ...
      rw [hfiyx]
      have hgf : (f.comp g hfg).isBijective :=
        aux₁ f g hfg hf hg
      ...
      have hgfxz : (f.comp g hfg).eval x hxgfdom = z := by
        have hfxgdom : f.eval x hxfdom ∈ g.domain := by
          ...
          exact MyFun.eval_codomain f x hxfdom
        rw [MyFun.comp.eval f g hfg x hxfdom hfxgdom hxgfdom]
        have hgeq : g.eval (f.eval x hxfdom) hfxgdom = g.eval y hygdom :=
...
example
  ...
  · constructor
    ...
    · intro x hxYZXYdom hxXZdom
      have hxXYdom : x ∈ (ι X Y hXY).domain := by
        dsimp only [ι]
        dsimp only [MyFun.from_fun]
        dsimp only [ι] at hxXZdom
        dsimp only [MyFun.from_fun] at hxXZdom
        exact hxXZdom
      have hXYxYZdom : (ι X Y hXY).eval x hxXYdom ∈ (ι Y Z hYZ).domain := by
...
example
  ...
  · constructor
    ...
    · intro x hxfdom hxfAdom
      ...
      have hxeval : x = (ι_id A).eval x hxAdom := by
        dsimp only [ι_id]
        dsimp only [ι]
  ...
  · constructor
    ...
    · intro x hxfdom hxBfdom
      have hfxBdom : f.eval x hxfdom ∈ (ι_id B).domain := by
        have hdomeq : (ι_id B).domain = f.codomain := by
          dsimp only [ι_id]
          dsimp only [ι]
          dsimp only [MyFun.from_fun]
    ...
    · dsimp only [MyFun.eq]
      ...
      · constructor
        ...
        · intro x hxhXXYdom hxfdom
          have hXXYcodomhdom : (ι X (X ∪ Y) hXXY).codomain = h.domain := by
            dsimp only [ι]
            dsimp only [MyFun.from_fun]
          have hxXXYdom : x ∈ (ι X (X ∪ Y) hXXY).domain := by
            dsimp only [ι]
            dsimp only [MyFun.from_fun]
            rw [← hfdom]
            exact hxfdom
          have hXXYxhdom :
            ...
            have hdomeq : h.domain = (ι X (X ∪ Y) hXXY).codomain := by
              dsimp only [ι]
              dsimp only [MyFun.from_fun]
            rw [hdomeq]
          ...
          have hheval :
              h.eval ((ι X (X ∪ Y) hXXY).eval x hxXXYdom) hXXYxhdom =
                h.eval x hxhdom := by
            have hιeval : (ι X (X ∪ Y) hXXY).eval x hxXXYdom = x := by
              dsimp only [ι]
          ...
          have hfxhcodom : f.eval x hxfdom ∈ h.codomain := by
            have hcodeq : h.codomain = f.codomain := by
              dsimp only [h]
              exact Eq.symm hfcodom
            rw [hcodeq]
            exact MyFun.eval_codomain f x hxfdom
          have hprop : h.prop x (f.eval x hxfdom) := by
            dsimp only [h]
            have hxX : x ∈ X := by
              rw [hfdom] at hxfdom
              exact hxfdom
            rw [dif_pos hxX]
          have hfeq : f.eval x hxfdom = h.eval x hxhdom :=
      ...
      · constructor
        ...
        · constructor
          ...
          · intro x hxhYXYdom hxgdom
            have hYXYcodomhdom : (ι Y (X ∪ Y) hYXY).codomain = h.domain := by
              dsimp only [ι]
              dsimp only [MyFun.from_fun]
            have hxYXYdom : x ∈ (ι Y (X ∪ Y) hYXY).domain := by
              dsimp only [ι]
              dsimp only [MyFun.from_fun]
              rw [← hgdom]
              exact hxgdom
            have hYXYxhdom :
              ...
              have hdomeq : h.domain = (ι Y (X ∪ Y) hYXY).codomain := by
                dsimp only [ι]
                dsimp only [MyFun.from_fun]
              rw [hdomeq]
            ...
            have hheval :
                h.eval ((ι Y (X ∪ Y) hYXY).eval x hxYXYdom) hYXYxhdom =
                  h.eval x hxhdom := by
              have hιeval : (ι Y (X ∪ Y) hYXY).eval x hxYXYdom = x := by
                dsimp only [ι]
            ...
            have hxgxhcodom : g.eval x hxgdom ∈ h.codomain := by
              have hcodeq : h.codomain = g.codomain := by
                dsimp only [h]
                exact Eq.symm hgcodom
              rw [hcodeq]
              exact MyFun.eval_codomain g x hxgdom
            have hprop : h.prop x (g.eval x hxgdom) := by
              dsimp only [h]
              have hxY : x ∈ Y := by
                rw [hgdom] at hxgdom
                exact hxgdom
              ...
              rw [dif_neg hxnX]
              rw [dif_pos hxY]
            have hgeq : g.eval x hxgdom = h.eval x hxhdom :=
      ...
      · intro h' hh'dom hh'codom hh'f hh'g
        ...
        · constructor
          ...
          · intro x hxh'dom hxhdom
            have hprop : h.prop x (h'.eval x hxh'dom) := by
              dsimp only [h]
              by_cases hxX : x ∈ X
              · rw [dif_pos hxX]
                ...
                have hxeval : x = (ι X (X ∪ Y) hXXY).eval x hxh'XXY := by
                  dsimp only [ι]
              ...
              · rw [dif_neg hxX]
                ...
                have hxeval : x = (ι Y (X ∪ Y) hYXY).eval x hxh'YXY := by
                  dsimp only [ι]
            ...
            have hh'xhcodom : h'.eval x hxh'dom ∈ h.codomain := by
              have hcodeq : h.codomain = h'.codomain := by
                dsimp only [h]
